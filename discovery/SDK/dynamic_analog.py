...
def measure(device_handle,channel):
    ...
    dwf.FDwfAnalogInConfigure(device_handle.handle, c_bool(False), c_bool(False))
 
    # read data to an internal buffer
    dwf.FDwfAnalogInStatus(device_handle.handle, c_bool(False), c_int(0))
 
    # extract data from that buffer
    voltage = c_double()   # variable to store the measured voltage
    dwf.FDwfAnalogInStatusSample(device_handle.handle, c_int(channel - 1), byref(voltage))
